  ...
  return (
    ...
    >
      <nav className={`pointer-events-auto flex items-center justify-between transition-all duration-300 w-full ${isDevMode ? 'px-6 py-2 bg-dev-dim/90 backdrop-blur-md border-b border-dev-primary/20 shadow-none' : 'max-w-5xl px-6 py-3 bg-white/70 backdrop-blur-md rounded-2xl shadow-[0_4px_30px_rgba(0,0,0,0.02)]'}`}>
        ...
        <div className={`flex items-center gap-6 ${isDevMode ? 'text-xs font-mono text-dev-soft' : 'text-sm font-medium text-foreground'}`}>
          <div className={`hidden md:flex items-center gap-1 p-1 rounded-full border transition-colors ${isDevMode ? 'bg-[#000000] border-dev-primary/30 mr-2' : 'bg-muted/50 border-border mr-2'}`}>
            <button 
              onClick={() => isDevMode && toggleDevMode()}
              className={`px-3 py-1 rounded-full transition-all text-xs font-sans ${!isDevMode ? 'bg-background shadow-sm text-foreground' : 'text-dev-soft/50 hover:text-dev-primary'}`}
            >
              Visual
            </button>
            <button 
              onClick={() => !isDevMode && toggleDevMode()}
              className={`px-3 py-1 rounded-full transition-all text-xs font-mono font-medium tracking-tight ${isDevMode ? 'bg-dev-dim text-dev-primary shadow-[0_0_10px_rgba(0,255,159,0.2)]' : 'text-muted-foreground hover:text-foreground'}`}
            >
              SYS.DEV
            </button>
          </div>
          <button onClick={() => scrollTo('work')} className="hover:opacity-60 transition-opacity">Projects</button>
